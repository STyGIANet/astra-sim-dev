...
def completionTime(a: int, b: int, m: float, beta: float, delta: float) -> float:
    if b<a or beta == 0.0:
        return 0.0

    if a==b:
        return delta*1 + beta * (m/(2**a))
    else:
        propcost = 0
        trancost = 0
        # step indexed from 0
        for step in range(a-1, b):
            rho = np.abs(np.sum([(-2)**i for i in range (0,step+1)]))
            propcost += delta*rho
            trancost += beta*(m/(2**(step+1)))*rho
...
def computeSchedule(s: int, k: int, m: int = 1, beta: float = 1.0, delta: float = 1.0) -> Tuple[float, List[int]]:
    ...
    for a in range(1, s+1):
        ...
        comm = completionTime(a, s, m, beta,delta)
        prop = 0
        # print(beta, m, a, s)
    ...
    for t in range(1, k + 1):
        for a in range(1, s+1):
            ...
            # for b in reversed(range(a,s+1)):
                comm = completionTime(a, b, m, beta,delta)
                prop = 0
...
def main():
    ...
    with open(file_name, "w") as f:
        reconf_list = f"Reconfiguration List: {finalReconfList}\n"
        reconf_cost = f"Total reconfig cost: {len(finalReconfList) * reconf * 1e9} ns\n"
        content_str = '\n'.join(content) + '\n'
        reverse_str = '\n'.join(reversed(reverseContent)) + '\n'
    
        output_content = reconf_list + reconf_cost + content_str + reverse_str
