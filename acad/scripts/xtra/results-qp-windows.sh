	...
	for ALG in ${ALGS[@]};do
		...
		for WINDOW in ${WINDOWS[@]};do
			...
			for ALLREDUCE_ALG in ${ALLREDUCE_ALGS[@]};do
				...
				FCT_FILE=${RESULTS_DIR}/fct-leaf-spine-${NUM_NODES}-${ROUTING}-${APP_LOADBALANCE_ALG}-${ALLREDUCE_ALG}-${MSG_SIZE}-${WINDOW}.txt
				
				CCT=$(cat $FCT_FILE | tail -n1 | awk '{print $9}')
				QP_COUNT=$(cat $FCT_FILE | sort -nk12 | tail -n1 | awk '{print $12}')

				echo "$ALG $MSG_SIZE $WINDOW $CCT $QP_COUNT"
