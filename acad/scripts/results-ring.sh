            ...
            for ALPHA_DELTA_ID in ${!ALPHA_DELAYS[@]};do
                ...
                for ALPHA_R in ${RECONFIG_DELAYS[@]};do
                    ...
                    for IDX in ${!MSG_NAMES[@]};do
                        ...
                        TOPO_FILE=$OPTICAL_ROUTING_DIR/harvest-${ALG}RD-$N.json
                        FCT_FILE=${RESULTS_DIR}/fct/fct-${HARVEST_TOPO}-${N}-${TYPE}-${APP_LOADBALANCE_ALG}-${ALG}-${MSG_NAME}-${ALPHA}-${DELTA}-${BW}.txt
